        ...
        <div class="toolbar" id="kt_toolbar">
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                    ...
                    <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Edit Report</h1>
            ...
            <div id="kt_content_container" class="container-xxl">
                ...
                <div class="card">
                    ...
                    <div class="card-body py-4">
                        <form id="kt_edit_report_form" novalidate="novalidate">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row mb-7">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select form-control-solid" data-control="select2" data-placeholder="Select Client Type" name="client_type" required>
                                        <option></option>
                                        <option value="internal" {{ $report->client_type === 'internal' ? 'selected' : '' }}>Internal</option>
                                        <option value="external" {{ $report->client_type === 'external' ? 'selected' : '' }}>External</option>
                                    </select>
                                    ...
                                    <div class="d-flex gap-2">
                                        <select class="form-select form-control-solid" data-control="select2" data-placeholder="Select Client" name="client_id" id="client_id">
                                            ...
                                            @foreach($clients as $client)
                                                <option value="{{ $client->id }}" {{ $report->client_id == $client->id ? 'selected' : '' }}>{{ $client->company_name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="d-flex gap-4">
                                        <label class="form-check form-check-custom form-check-solid">
                                            <input class="form-check-input" type="radio" name="show_report_code" value="Yes" {{ $report->show_report_code === 'Yes' ? 'checked' : '' }} />
                                            <span class="form-check-label">Yes</span>
                                        </label>
                                        <label class="form-check form-check-custom form-check-solid">
                                            <input class="form-check-input" type="radio" name="show_report_code" value="No" {{ $report->show_report_code === 'No' ? 'checked' : '' }} />
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select form-control-solid" data-control="select2" data-placeholder="Select Tech. Manager" name="assign_id" required>
                                        ...
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ $report->assign_id == $employee->id ? 'selected' : '' }}>{{ $employee->first_name }} {{ $employee->last_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            {{-- <div class="row mb-7">
                                <div class="col-md-6">
                                    <label class="required fs-6 fw-bold mb-2">Show Logo *</label>
                                    <div class="d-flex gap-4">
                                        <label class="form-check form-check-custom form-check-solid">
                                            <input class="form-check-input" type="radio" name="show_logo" value="Yes" {{ $report->show_logo === 'Yes' ? 'checked' : '' }} />
                                            <span class="form-check-label">Yes</span>
                                        </label>
                                        <label class="form-check form-check-custom form-check-solid">
                                            <input class="form-check-input" type="radio" name="show_logo" value="No" {{ $report->show_logo === 'No' ? 'checked' : '' }} />
                                            <span class="form-check-label">No</span>
                                        </label>
                                    </div>
                                    <div class="fv-plugins-message-container invalid-feedback" id="show_logo-error"></div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select form-control-solid" data-control="select2" data-placeholder="Select Employee" name="employee_id">
                                        <option></option>
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ $report->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->first_name }} {{ $employee->last_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="d-flex justify-content-end">
                                ...
                                <button type="submit" id="kt_edit_report_submit" class="btn btn-primary">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            // Form submission
            $('#kt_edit_report_form').on('submit', function(e) {
                e.preventDefault();

                var formData = new FormData(this);
                const submitButton = document.querySelector('#kt_edit_report_submit');
                submitButton.setAttribute('data-kt-indicator', 'on');
                submitButton.disabled = true;

                $.ajax({
                    url: "{{ route('batches.results.reports.update', ['batch' => $batch->id, 'result' => $reportVersion->id, 'report' => $report->id]) }}",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
